    ...
    public function saveAction() {
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                if (Mage::helper('brand')->urlKey() != '') {
					$currentUrl = Mage::helper('brand')->urlKey();
					$c = strrpos($currentUrl, '.');
					if($c){
						$nameCategory = substr($currentUrl, 0, $c);
						$html = substr($currentUrl, $c + 1);
						$urlRewrite->setRequestPath($nameCategory . '/' . $model->getUrlKey() . '.' . $html);
					}else{
						$urlRewrite->setRequestPath($currentUrl . '/' . $model->getUrlKey());
					}
