...
class MGS_Brand_IndexController extends Mage_Core_Controller_Front_Action {
    ...
    public function viewAction() {
		...
		if($category && $category->getMetaTitle()){
			$title = $category->getMetaTitle();
		}else{
			$title = $model->getTitle();
		}
		if($category && $category->getMetaKeywords()){
			$metaKeywords = $category->getMetaKeywords();
		}else{
			$metaKeywords = $model->getMetaKeywords();
		}
		if($category && $category->getMetaDescription()){
			$metaDescription = $category->getMetaDescription();
		}else{
			$metaDescription = $model->getMetaDescription();
		}
        $this->getLayout()->getBlock('head')->setTitle($this->__($title));
        $this->getLayout()->getBlock('head')->setKeywords($this->__($metaKeywords));
        $this->getLayout()->getBlock('head')->setDescription($this->__($metaDescription));
		$this->getLayout()->getBlock('head')->addLinkRel('canonical', $category->getUrl());
		...
		$this->getLayout()->getBlock('root')->setBrand(1);
